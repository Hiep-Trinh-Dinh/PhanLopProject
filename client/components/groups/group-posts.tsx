...
export default function GroupPostFeed({ groupId }: GroupPostFeedProps) {
  ...
  const fetchGroupPosts = async () => {
    try {
      setLoading(true);
      console.log(`Đang lấy bài viết cho nhóm ${groupId}`);
      const response = await PostApi.getGroupPosts(groupId, 0, 20);
      console.log("Kết quả:", response.content?.length || 0, "bài viết");

      if (!response.content || response.content.length === 0) {
        setError("Nhóm này chưa có bài viết nào.");
        setPosts([]);
        setLoading(false);
        return;
      }
      ...
      console.error("Lỗi khi tải bài viết nhóm:", err);
      if (err.message?.includes("Unauthorized")) {
        setError("Vui lòng đăng nhập lại để xem bài viết");
        window.location.href = "/login";
      } else if (err.message?.includes("403")) {
        setError("Bạn không có quyền xem bài viết của nhóm này.");
      } else if (err.message?.includes("404")) {
        setError("Nhóm không tồn tại hoặc không tìm thấy bài viết.");
      } else {
  ...
  useEffect(() => {
    console.log(`GroupPostFeed: Đã nhận groupId=${groupId}`);
    fetchGroupPosts();
  }, [groupId, refreshFlag]);
