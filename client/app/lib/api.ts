...
async function fetchWithAuth<T>(path: string, options: RequestInit = {}): Promise<T> {
  const maxRetries = 3; // Tăng số lần thử lại
  let retries = 0;
  ...
  // Kiểm tra kết nối đến server API trước
  const isServerAvailable = await checkServerConnectivity(API_BASE_URL);
  ...
  while (retries <= maxRetries) {
    try {
      console.log(`Fetching (attempt ${retries + 1}/${maxRetries + 1}): ${url}`);
